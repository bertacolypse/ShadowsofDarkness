...
function display_your_command()
{
	writelog('');
	writelog(String.format('Your command,`0${0}`2? : ', player));
}
...
function save_data()
{
	if (typeof _bbs_save !== 'undefined') // running inside a bbs
	{
		...
		_bbs_save_type('lord', 'handle',  player);
		_bbs_save();
	}
...
function load_savedata()
{
	if (typeof _bbs_load !== 'undefined') // running inside a bbs
	{
		...
		player         = _bbs_load_type('lord', 'UNSET', 'handle');
		return 1;
	}
...
function talk_dbartender()
{
	...
	writelog('(R)eturn');
	writelog('');
	writelog('Your command? (? for menu) [R] : ');
}
		...
		else if (key == 10) // enter
		{
			...
			else
			{
				...
				if (n == 1)
				{
					...
					let bk = inc_player_value(player, 'bank', n, MAX_VALUE);
					set_player_value(player, 'gold', 0);
					
					writelog('');
					writelog('Done! '+n+' deposited.');
					writelog('');
				...
				else
				{
					...
					let gd = dec_player_value(player, 'gold', n, 0);
					if (player_value_not_found == 1) { gd = 0; }
